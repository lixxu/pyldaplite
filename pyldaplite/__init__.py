...
class PyLDAPLite(object):
    ...
    def connect(self, username, password):
        if self.server.startswith("ldaps:"):
            ...
            self.conn.set_option(ldap.OPT_PROTOCOL_VERSION, self.version)
    ...
    def search_by(self, filter_value, **kwargs):
        ...
        result_id = self.conn.search(
            self.base_dn, self.search_scope, filter_value, self.retrive_attrs,
        )
        results = []
        while 1:
    ...
    def search_email(self, email, limit=0):
        return self.search_by(
            COMMON_FILTERS["email"].format(email), limit=limit
        )
    ...
    def search_phone(self, phone, limit=0):
        return self.search_by(
            COMMON_FILTERS["phone"].format(phone), limit=limit
        )

    def search_mobile(self, mobile, limit=0):
        return self.search_by(
            COMMON_FILTERS["mobile"].format(mobile), limit=limit
        )

    def search_number(self, number, limit=0):
        return self.search_by(
            COMMON_FILTERS["number"].format(number), limit=limit
        )

    def search_display(self, display_name, limit=0):
        return self.search_by(
            COMMON_FILTERS["display"].format(display_name), limit=limit
        )
